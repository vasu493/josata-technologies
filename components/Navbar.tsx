...
import { DigitalMenuContent } from './menu/DigitalMenuContent';
import { StandardMenuContent } from './menu/StandardMenuContent';
import { IndustriesMenuContent } from './menu/IndustriesMenuContent';
...
export const Navbar: React.FC<Props> = ({ onVibeChange }) => {
  ...
  const handleFunctionSelect = (funcName: string, category: string) => {
    if (onVibeChange) {
      ...
      setActiveMenu(null); // Close the menu on select
    }
  };
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50">
      <div className="bg-white border-b border-slate-100 h-20 relative z-50 shadow-sm">
        <div className="max-w-[1440px] mx-auto px-8 h-full flex items-center justify-between">
          <div className="flex items-center gap-6 h-full">
            <button onClick={() => setIsSideMenuOpen(true)} className="flex flex-col gap-1 w-6 group px-1" aria-label="Open Menu">
              <div className="w-full h-[1.5px] bg-black transition-all group-hover:w-3/4" />
              <div className="w-full h-[1.5px] bg-black" />
              <div className="w-full h-[1.5px] bg-black transition-all group-hover:w-3/4" />
            </button>
            <a href="#" onClick={() => onVibeChange?.('home')} className="text-xl md:text-2xl font-bold tracking-tighter text-[#001b3d] flex items-center gap-1 whitespace-nowrap">
              ...
              {NAV_ITEMS.map((item) => (
                ...
                >
                  <span className={`text-[14px] font-semibold transition-colors relative z-10 ${
                    activeMenu === item.label ? 'text-[#001b3d]' : 'text-slate-700 hover:text-[#001b3d]'
                  }`}>
                    {item.label}
                  </span>
                  <AnimatePresence>
                    {activeMenu === item.label && (
                      <div className="absolute inset-0 flex flex-col items-center justify-end pointer-events-none">
                        <motion.div initial={{ width: 0 }} animate={{ width: "80%" }} exit={{ width: 0 }} transition={{ type: "spring", stiffness: 300, damping: 25 }} className="h-[3px] bg-[#001b3d] mb-2" />
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: 0.2, ease: "easeOut" }} className="absolute -bottom-1">
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="bg-[#001b3d] text-white px-6 py-2 rounded text-[13px] font-bold uppercase tracking-wider hover:bg-slate-800 transition-all shadow-sm">Get Started</button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {activeMenu && currentItem?.dropdown && (
          <motion.div
            ...
            onMouseLeave={() => setActiveMenu(null)}
            className={`absolute top-20 left-0 w-full min-h-[500px] shadow-2xl z-40 overflow-hidden ${
              activeMenu === 'Digital' || activeMenu === 'Industries' ? 'bg-black text-white' : 'bg-[#001b3d] text-white'
            }`}
          >
            {activeMenu === 'Digital' ? (
              ...
              <StandardMenuContent item={currentItem} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isSideMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={handleSideMenuClose} className="fixed inset-0 bg-black/20 backdrop-blur-[2px] z-[100]" />
